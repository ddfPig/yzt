<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:75:"C:\phpStudy\PHPTutorial\WWW\tp/application/index\view\purchase\purEdit.html";i:1538208342;s:74:"C:\phpStudy\PHPTutorial\WWW\tp\application\index\view\common\userinfo.html";i:1538038290;}*/ ?>
                ...
                <div class="row">
                    <ul>
                        <li>
                            ...
                            <input type="text" value="<?php echo $pur['SupplyName']; ?>"  name="supplier_name"  readonly="readonly" style="border:1px solid #1BBC9B" /></li>
                        <li><label>供货商编码</label><input type="text" readonly="readonly" name="supplier_code" value="<?php echo $pur['supplier_code']; ?>" id="supplier_code" /></li>
                        <li><label>单据编号</label><input type="text" readonly="readonly" name="purnumbers" value="<?php echo $pur['purnumbers']; ?>"  id="purnumbers" /></li>
                        <li><label>创建日期</label><input type="text"  data-options="editable:false" name="create_time" value="<?php echo $pur['create_time']; ?>" class="easyui-datebox"  style="width:170px"/></li>
                        <li><label>确认日期</label><input type="text"  data-options="editable:false" name="confirm_time" value="<?php echo $pur['confirm_time']; ?>" class="easyui-datebox"  style="width:170px"/></li>
                        <li><label>操作人</label><input type="text" readonly="readonly" value="<?php echo $pur['operater']; ?>" name="operater"  /></li>
                    </ul>
                    <ul>
                        <li><label>采购总金额</label><input type="text" value="<?php echo $pur['purtotal']; ?>" readonly="readonly" id="OrderAccount" name="purtotal"/></li>
                        <li><label>应付总金额</label><input type="text" value="<?php echo $pur['puryftotal']; ?>" readonly="readonly" id="PayAccount" name="puryftotal"/></li>
                        <li><label>到货日期</label><input type="text" data-options="editable:false" value="<?php echo $pur['daohuotime']; ?>" class="easyui-datebox"  name="daohuotime" style="width:170px" /></li>
                        <li><label>随货同行单号</label><input type="text" name="SHDH" value="<?php echo $pur['SHDH']; ?>" /></li>
                        <li><label>联系人</label><input type="text" name="contacts_person" value="<?php echo $pur['contactor']; ?>" id="contacts_person" /></li>
                        <li><label>联系电话</label><input type="text" name="contacts_phone" value="<?php echo $pur['contactor_phone']; ?>" id="contacts_phone" /></li>
                    </ul>

                    <ul>
                        <li><label>备注</label><textarea name="info"><?php echo $pur['info']; ?></textarea></li>
...
<input type="hidden" id="SupplyID" name="SupplyID" value="<?php echo $pur['SupplyID']; ?>" />
<input type="hidden" id="State" name="State" />
<input type="hidden" id="detail" name="detail" />
<input type="hidden" id="pid" name="pid" value="<?php echo $pur['pid']; ?>" />
<input type="hidden" id="puid" name="puid" value="<?php echo $pur['puid']; ?>" />
...
<script>
    ...
    var mustcolumnstyle =  'background-color:#eeeeee;';
    var purnumbers = "<?php echo $pur['purnumbers']; ?>";
    $(function(){
        $('#dlg').dialog('close');

        $('#rangegrid').datagrid({
            methord: 'get',
            url: "<?php echo url('index/purchase/getStore'); ?>?pid="+purnumbers,
            sortName: 'store_id',
            sortOrder: 'desc',
            idField: '',
            pageSize: 50,
            ...
            columns: [[
                {
                    field: 'store_id', title: 'store_id', hidden: 'true'
                },
            ...
            $.messager.confirm('删除提示', '确定要删除吗？', function (r) {
                if (r) {
                    for (var i = 0; i < lengths; i++) {
                        ...
                        //所选行id
                        $('#rangegrid').datagrid('getRows')[index]['store_id'];
                    }
                }
                else {
                            ...
                            if (param.field == "uname") {
                                ...
                                $(ed.target).bind('dblclick', function (e) {
                                    ...
                                    $('#dlg').dialog({
                                        ...
                                        top: 100,
                                        left: 200,
                                        href: "<?php echo url('index/purchase/SelPro'); ?>?dwid=" + $("#SupplyID").val(),
                                        buttons: [{
                                            text: '确定',
                                            iconCls: 'icon-ok',
                                            handler: function () {
                        ...
                        $('.datagrid-editable .textbox,.datagrid-editable .datagrid-editable-input,.datagrid-editable .textbox-text').blur(function () {
                            ...
                            if (param.field == "price" || param.field == "pnumber" || param.field == "innumber") {
                                if (param.field == "pnumber") {
                                    ...
                                    money = $("#rangegrid").datagrid('getRows')[param.index]['price'];
                                    //商品数量改变时商品单价相关计算
                                    var spmoney = parseFloat(count ? count : '0') * parseFloat(money ? money : '0');
                                    $("#rangegrid").datagrid('getRows')[param.index]['MoneyAccount'] = parseFloat(spmoney.toFixed(2));
    ...
    function Save() {
        ...
        if (IsSubmit) {
            IsSubmit = false;
            $.post("<?php echo url('index/purchase/purUpdate'); ?>", $("form").serialize(),
                function (result) {
