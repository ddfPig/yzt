<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:88:"C:\phpStudy\PHPTutorial\WWW\tp/application/index\view\helpdocument\helpdocument_add.html";i:1538290927;s:74:"C:\phpStudy\PHPTutorial\WWW\tp\application\index\view\common\userinfo.html";i:1538038290;}*/ ?>
            ...
            <div class="form_table2">
                <div class="row">
                    <form id="dd">
                        ...
                        <li><label>标题</label><input type="text" name="bzwdgl_title"/></li>
                        <li><label>自定义时间</label><input type="text"  name="bzwdgl_customize_time" class="easyui-datebox" editable="false" style="width:174px"/></li>
                        <li><label>内容</label><textarea style="width:170px;height:80px" name="bzwdgl_contents"></textarea></li>
                    </ul>
                    </form>
                </div>
            </div>
...
<script>
    ...
    function Bindqylx() {
        $("#bzwdgl_state").html("");
        $("#bzwdgl_state").html(option);
        $.getJSON("<?php echo url('index/helpdocument/type'); ?>", {}, function (data) {
            $.each(data, function (i, item) {
                $("#bzwdgl_state").append(" <option value='" + item.id + "'>" + item.name + "</option>");
            });
        })
    };
    ...
    function add(){
        $.ajax({
            url:"<?php echo url('index/helpdocument/adddobutton'); ?>",
            type:"POST",
            data:$('#dd').serialize(),
