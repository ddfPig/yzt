<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:72:"C:\phpStudy\PHPTutorial\WWW\tp/application/index\view\receipt\index.html";i:1538269414;s:74:"C:\phpStudy\PHPTutorial\WWW\tp\application\index\view\common\userinfo.html";i:1538038290;}*/ ?>
    ...
    <style>
        .breadcrumb {
            ...
            <div class="margin-bottom-5">
                <a class="easyui-linkbutton" data-options="iconCls:'icon-add'" href="<?php echo url('index/receipt/receiptadd'); ?>">新建</a>
...
<script type="text/javascript" >
    ...
    $(function(){
        ...
        grid = $('#rangegrid').datagrid({
            methord: 'get',
            url: "<?php echo url('index/receipt/ini'); ?>",
            sortOrder: 'desc',
            idField: 'id',
            pageSize: 50,
            frozenColumns: [[
            ...
            columns: [[
                {
                    field: 'skfs_state', title: '状态'
                },
                {
                    field: 'skfs_name', title: '名称'
                },
                {
                ...
                {
                    field: 'skfs_issystem', title: '主键', hidden:"hidden",
                }
            ]],
            pagination: true,
            rownumbers: true,
            ...
            onDblClickCell: function (index, field, value) {
                ...
                var issystem = grid.datagrid('getRows')[index]['skfs_issystem'];
                if (issystem != 1){
                    window.location.href = '/index.php/index/receipt/edit' +
                        '?id=' + grid.datagrid('getRows')[index]['id'];
                }else {
                    $.messager.alert("提示", "此项为默认项无法修改！",'warning');
                }
        ...
        $("#dc").click(function (){
            ...
            $.messager.confirm('确认', '您确认要导出吗？', function (r) {
                if (r) {
                    window.location.href = "<?php echo url('index/receipt/daochu'); ?>";
                }
            });
        });
        ...
        function UpdateList() {
            $.ajax({
                url:"<?php echo url('index/receipt/sort'); ?>",
                type: "post",
                data: $("form").serialize(),
                success: function (s) {
                    var results = eval('('+s+')');
        ...
        $('#xg').click(function () {
            ...
            var issystem = rows[0]['skfs_issystem'];
            if (issystem == 1){
                $.messager.alert("提示", "此项为默认项无法修改！",'warning');
                return false;
            }
            ...
            window.location.href = ' /index.php/index/receipt/edit?id=' + id;
        });
        ...
        $('#qy').click(function(){
            ...
            $.ajax({
                url: "<?php echo url('index/receipt/qy'); ?>",
                type: "post",
                data: {ids:ids},
        ...
        $('#ty').click(function(){
            ...
            for (var i = 0; i < rows.length; i++) {
                ids.push(rows[i].id);
                skfs_issystem.push(rows[i].skfs_issystem);
            }
            $.ajax({
                url: "<?php echo url('index/receipt/ty'); ?>",
                type: "post",
                data: {ids:ids,skfs_issystem:skfs_issystem},
                success: function (result) {
                    var results = eval('('+result+')');
                    if (results.error == 1) {
                        $.messager.alert('提示',results.msg,'info');
