<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:73:"C:\phpStudy\PHPTutorial\WWW\tp/application/index\view\customer\index.html";i:1538269401;s:74:"C:\phpStudy\PHPTutorial\WWW\tp\application\index\view\common\userinfo.html";i:1538038290;}*/ ?>
    ...
    <style>
        .breadcrumb {
            ...
            <div class="margin-bottom-5">
                ...
                <a class="easyui-linkbutton" data-options="iconCls:'icon-add'" href="<?php echo url('index/customer/customeradd'); ?>">新建</a>
                ...
                <select style="height:24px;line-height:24px; width:auto" id="status" name="status" >
                    <option value="-1">按状态筛选</option>
                    <option value="1">已启用</option>
                    <option value="0">已停用</option>
                </select>
                <input type="text" placeholder="请输入查询关键字" style="width:200px" id="keyword" name="keyword">
                <a class="easyui-linkbutton" data-options="iconCls:'icon-search'" id="search">查找</a>
                <?php endif; ?>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        grid = $('#rangegrid').datagrid({
            methord: 'get',
            url: "<?php echo url('index/customer/ini'); ?>",
            sortOrder: 'desc',
            idField: 'id',
            pageSize: 50,
            frozenColumns: [[
            ...
            columns: [[
                {
                    field: 'pkfgl_state', title: '状态'
                },
                {
                    field: 'pkfgl_id', title: '编码'
                },
                {
                    field: 'pkfgl_username', title: '登录用户名'
                },
                {
                    field: 'pkfgl_name', title: '姓名'
                },
                {
                    field: 'pkfgl_phone', title: '手机号'
                },
                {
                    field: 'pkfgl_landline', title: '固定电话'
                },
                {
                    field: 'pkfgl_qq', title: 'QQ'
                },
                {
                    field: 'pkfgl_email', title: '邮箱'
                },
                {
                    field: 'pkfgl_create_time', title: '创建时间'
                },
                {
                    field: 'pkfgl_last_time', title: '最后修改时间'
                },
                {
            ...
            onDblClickCell: function (index, field, value) {
                window.location.href = '/index.php/index/customer/edit' +
        ...
        $("#dc").click(function (){
            ...
            $.messager.confirm('确认', '您确认要导出吗？', function (r) {
                ...
                if (r) {
                    ...
                    window.location.href =  "<?php echo url('index/customer/daochu'); ?>?status="+status+"&keyword="+keyword;
                }
            });
        });
        ...
        $('#qy').click(function(){
            ...
            $.ajax({
                url: "<?php echo url('index/customer/qy'); ?>",
                type: "post",
                data: {ids:ids},
                success: function (s) {
                    if (s.error == 1) {
        ...
        $('#ty').click(function(){
            ...
            $.ajax({
                url: "<?php echo url('index/customer/ty'); ?>",
                type: "post",
                data: {ids:ids},
                success: function (s) {
                    if (s.error == 1) {
