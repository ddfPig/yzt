<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:74:"C:\phpStudy\PHPTutorial\WWW\tp/application/index\view\clerk\clerk_add.html";i:1538290821;s:74:"C:\phpStudy\PHPTutorial\WWW\tp\application\index\view\common\userinfo.html";i:1538038290;}*/ ?>
...
function  add(){
	...
	if(!(/^1[34578]\d{9}$/.test(clerk_phone))){ 
		$.messager.alert('职员管理','手机号码有误，请重填','warning');		
        return false; 
    } 
	...
	var   len=clerk_pass.length;
	
	if(len<6){
		$.messager.alert('职员管理','密码最少位数不能小于6位','warning');
		return;
	}
	if(len>20){
		$.messager.alert('职员管理','密码最多位数不能大于20位','warning');
		return;
	}
